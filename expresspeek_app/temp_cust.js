...
function getAddressBookEntries() {
    ...
    return fetch('{{ route("customer.address-book.api.index", [], false) }}', {
...
function updateShipmentTypeSelectionStyles() {
    ...
    document.querySelectorAll('[data-shipment-type-card]').forEach(card => {
        ...
        if (panel) {
            panel.style.borderColor = isSelected ? '#8b5cf6' : '#e2e8f0';
...
function loadCarriers() {
    ...
    fetch(`{{ $guestMode ? route('shipment.carriers.available', [], false) : route('customer.carriers.api.index', [], false) }}?country_code=${encodeURIComponent(code || country)}&country=${encodeURIComponent(country)}&weight=${encodeURIComponent(totalWeight || 1)}&type=${encodeURIComponent(shipmentType)}`, {
        credentials: 'same-origin',
        headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
    })
    .then(r => {
        ...
        carriers.forEach(c => {
            ...
            card.innerHTML = `
                ...
                <div class="relative border-2 rounded-2xl p-5 transition-all duration-200 peer-checked:border-violet-400 peer-checked:bg-violet-600/15 peer-checked:shadow-lg peer-checked:shadow-violet-900/20 hover:border-gray-500 hover:-translate-y-0.5"
                    style="border-color: ${String(selectedCarrierId) === String(c.id) ? '#8b5cf6' : '#e2e8f0'}; background: ${String(selectedCarrierId) === String(c.id) ? 'rgba(245, 243, 255, 0.96)' : '#ffffff'};">
                    <div class="absolute top-4 right-4 items-center gap-1 rounded-full bg-violet-500 px-2 py-1 text-[10px] font-semibold text-white shadow-sm hidden" data-carrier-selected-badge="${c.id}">
...
function updateCarrierSelectionStyles() {
    document.querySelectorAll('[data-carrier-card]').forEach(card => {
        ...
        if (panel) {
            panel.style.borderColor = isSelected ? '#8b5cf6' : '#e2e8f0';
            panel.style.background = isSelected ? 'rgba(245, 243, 255, 0.96)' : '#ffffff';
            panel.style.boxShadow = isSelected ? '0 14px 34px rgba(91, 33, 182, 0.12)' : '';
            panel.style.transform = isSelected ? 'translateY(-2px)' : '';
        }
    });
}
...
function submitShipment() {
    ...
    fetch('{{ $guestMode ? route("shipment.store", [], false) : route("customer.shipments.store", [], false) }}', {
        method: 'POST',
        credentials: 'same-origin',
        headers: {
...
function saveToAddressBook(prefix) {
    ...
    fetch('{{ route("customer.address-book.api.store", [], false) }}', {
        method: 'POST',
        credentials: 'same-origin',

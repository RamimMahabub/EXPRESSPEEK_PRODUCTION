        ...
        <div class="wizard-step active" data-step="1">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 relative">
                ...
                <div class="neon-card rounded-2xl p-6">
                    ...
                    @include('agent.partials.contact-form', ['prefix' => 'sender', 'showAddressBookPicker' => ! $guestMode])
                </div>
                ...
                <div class="neon-card rounded-2xl p-6">
                    ...
                    @include('agent.partials.contact-form', ['prefix' => 'receiver', 'showAddressBookPicker' => ! $guestMode])
                </div>
            </div>
        </div>
            ...
            <div class="neon-card rounded-2xl p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <label class="shipment-type-card block cursor-pointer" for="type_document" data-shipment-type-card="document">
                        <input type="radio" id="type_document" name="shipment_type" value="document" class="sr-only peer" onchange="onShipmentTypeChange()">
                        <div class="relative border-2 border-gray-700 rounded-2xl p-6 transition-all duration-200 peer-checked:border-violet-400 peer-checked:bg-violet-600/15 peer-checked:shadow-lg peer-checked:shadow-violet-900/20 hover:border-gray-600">
                            <div class="absolute top-4 right-4 inline-flex items-center gap-1 rounded-full bg-violet-500 px-2 py-1 text-[10px] font-semibold text-white shadow-sm hidden" data-shipment-type-badge="document">
                            ...
                            <h4 class="font-semibold text-white text-lg">Document</h4>
                            <p class="text-slate-400 text-sm mt-1">Letters, contracts, certificates, papers</p>
                            <p class="text-violet-300 text-xs mt-3 hidden" data-shipment-type-note="document">This shipment will be created as a document.</p>
                        </div>
                    </label>
                    <label class="shipment-type-card block cursor-pointer" for="type_non_document" data-shipment-type-card="non_document">
                        <input type="radio" id="type_non_document" name="shipment_type" value="non_document" class="sr-only peer" onchange="onShipmentTypeChange()" checked>
                        <div class="relative border-2 border-gray-700 rounded-2xl p-6 transition-all duration-200 peer-checked:border-violet-400 peer-checked:bg-violet-600/15 peer-checked:shadow-lg peer-checked:shadow-violet-900/20 hover:border-gray-600">
                            <div class="absolute top-4 right-4 inline-flex items-center gap-1 rounded-full bg-violet-500 px-2 py-1 text-[10px] font-semibold text-white shadow-sm hidden" data-shipment-type-badge="non_document">
                                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                                Selected
                            </div>
                            <div class="text-4xl mb-3">📦</div>
                            <h4 class="font-semibold text-white text-lg">Non-Document</h4>
                            <p class="text-slate-400 text-sm mt-1">Goods, merchandise, gifts, samples</p>
                            <p class="text-violet-300 text-xs mt-3 hidden" data-shipment-type-note="non_document">This shipment will be created as a non-document parcel.</p>
                        </div>
                    </label>
                </div>
...
<div id="addressBookPickerModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70 px-4 py-6">
    <div class="w-full max-w-3xl rounded-3xl border border-white/10 bg-gray-950 shadow-2xl shadow-black/40">
        <div class="flex items-start justify-between gap-4 border-b border-white/10 px-6 py-5">
            <div>
                <h3 class="text-lg font-semibold text-white">Select From Address Book</h3>
                <p class="mt-1 text-sm text-slate-400">Choose a saved contact to auto-fill sender or receiver details.</p>
            </div>
            <button type="button" onclick="closeAddressBookPicker()" class="text-slate-400 hover:text-white transition-colors">
        ...
        <div class="border-b border-white/10 px-6 py-4">
            <input id="addressBookPickerSearch" type="text" placeholder="Search name, company, city, country..."
                ...
                class="w-full rounded-2xl border border-gray-700 bg-gray-900 px-4 py-3 text-sm text-white placeholder-gray-600 focus:border-violet-500 focus:outline-none">
        </div>
        ...
        <div class="max-h-[65vh] overflow-y-auto px-6 py-5">
            ...
            <div id="addressBookPickerEmpty" class="hidden rounded-2xl border border-dashed border-white/10 px-6 py-12 text-center text-sm text-slate-400">
                No saved addresses found.
            </div>
            <div id="addressBookPickerLoading" class="rounded-2xl border border-dashed border-white/10 px-6 py-12 text-center text-sm text-slate-400">
        ...
        carriers.forEach(c => {
            ...
            card.innerHTML = `
                ...
                <div class="relative border-2 rounded-2xl p-5 transition-all duration-200 peer-checked:border-violet-400 peer-checked:bg-violet-600/15 peer-checked:shadow-lg peer-checked:shadow-violet-900/20 hover:border-gray-500 hover:-translate-y-0.5"
                    ...
                    <div class="absolute top-4 right-4 inline-flex items-center gap-1 rounded-full bg-violet-500 px-2 py-1 text-[10px] font-semibold text-white shadow-sm ${String(selectedCarrierId) === String(c.id) ? '' : 'hidden'}" data-carrier-selected-badge="${c.id}">
                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                        Selected
                    </div>
